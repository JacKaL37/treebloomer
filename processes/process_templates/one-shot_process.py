...
def one_shot_process_template(input_file: Path, subfolder: Path) -> Path:
    ...
    logging.info(f"Processing {input_file.stem} with {process_name}...")
    ...
    if output_file.exists():
        logging.info(f"Processed file {output_file} already exists. Skipping processing.")
        return output_file
    ...
    try:
        ...
        logging.info(f"Processed {input_file.stem} to {output_file}.")
        return output_file
