...
def generate_html_summary(input_file: Path, subfolder: Path) -> Path:
    logging.info(f"Generating HTML summary for {input_file.stem}...")
    
    original_stem = input_file.stem.rsplit('.', 1)[0]
    output_file = subfolder / f"{original_stem}.html"
    
    if output_file.exists():
        logging.info(f"HTML summary file {output_file} already exists. Skipping generation.")
        return output_file
...
<body>
    ...
    <script>
        ...
        logging.info(f"Generated HTML summary at {output_file}.")
        return output_file
