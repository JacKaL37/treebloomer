...
def compress_audio(audio_file: Path, subfolder: Path) -> Path:
    logging.info(f"Compressing audio file {audio_file.stem}...")
    ...
    if compressed_audio_file.exists():
        logging.info(f"Compressed audio file {compressed_audio_file} already exists. Skipping compression.")
    ...
    try:
        ...
        logging.info(f"Compressed audio file {audio_file.stem} to {compressed_audio_file}.")
        return compressed_audio_file
    except Exception as e:
        logging.error(f"Failed to compress audio file {audio_file}: {e}")
